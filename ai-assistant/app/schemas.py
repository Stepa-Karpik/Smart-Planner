...
AssistantMode = Literal["AUTO", "PLANNER", "COMPANION"]
ActionType = Literal[
    "create_event",
    "update_event",
    "delete_event",
...
ObservationType = Literal["gap_request", "failure_case", "feature_demand", "misunderstanding", "new_intent"]
...
class WeeklyObservationsRequest(BaseModel):
    days: int = Field(default=7, ge=1, le=60)

    model_config = {"extra": "forbid"}


class PendingKBRequest(BaseModel):
    limit: int = Field(default=100, ge=1, le=500)

    model_config = {"extra": "forbid"}


class WeeklyObservationsResponse(BaseModel):
    date_from: date
    date_to: date
...
class PendingKBResponse(BaseModel):
    items: list[dict[str, Any]]
