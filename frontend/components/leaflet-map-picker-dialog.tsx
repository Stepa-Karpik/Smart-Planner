...
interface LeafletMapPickerDialogProps {
  value: MapPoint | null
...
export function LeafletMapPickerDialog({ value, onSelect }: LeafletMapPickerDialogProps) {
  const { tr } = useI18n()
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      setLoading(true)
      setError(null)
      mapLogInfo("MapPicker init started", { open, provider: "leaflet" })
      ...
      try {
        ...
        const clickHandler = (event: LeafletMapEvent) => {
          ...
          mapLogInfo("MapPicker point selected", { lat, lon, provider: "leaflet" })
        ...
        mapLogError("MapPicker init failed", err, { provider: "leaflet" })
      } finally {
  ...
  return (
    <Dialog
      open={open}
      onOpenChange={(next) => {
        mapLogInfo("MapPicker dialog state changed", { open: next, provider: "leaflet" })
        setOpen(next)
      }}
    >
      <DialogTrigger asChild>
        <Button
          ...
          onClick={(event) => {
            event.stopPropagation()
            mapLogInfo("MapPicker trigger clicked", { provider: "leaflet" })
          }}
        >
